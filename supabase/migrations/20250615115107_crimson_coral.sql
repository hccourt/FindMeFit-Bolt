/*
  ...
  1. New Tables
    - `notifications`
      ...
      - `type` (text, constrained to specific values)
      - `title` (text)
      - `message` (text)
      - `data` (jsonb)
      - `read` (boolean)
  ...
  3. Functions
    ...
    - Trigger function for automatic booking notifications
*/
...
DROP FUNCTION IF EXISTS notify_booking_confirmed();
...
BEGIN
  ...
  IF NEW.status = 'confirmed' AND (OLD IS NULL OR OLD.status != 'confirmed') THEN
    ...
    PERFORM create_notification(
      ...
      'You have a new booking from ' || client_info.name || ' for "' || class_info.title || '".',
      jsonb_build_object(
        'class_id', NEW.class_id,
        'booking_id', NEW.id,
        'class_title', class_info.title,
